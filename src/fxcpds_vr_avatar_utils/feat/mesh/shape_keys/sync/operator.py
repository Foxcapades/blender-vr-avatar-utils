...
from .....utils import compatibility, streams
from .....utils.aliases import Context, Icon, OperatorReturn
from .....utils.bpy_wrappers import KeyRef
...
def register() -> None:
    import bpy
    bpy.utils.register_class(SyncOperator)


def unregister() -> None:
    from .....utils import silent_unregister_class
    silent_unregister_class(SyncOperator)
...
class SyncOperator(FxOperator):
    ...
    @staticmethod
    def is_runnable(obj: Object):
        return utils.is_mesh_with_shape_keys(obj)
...
class _SyncContext:
    def __init__(self, ctx: Context):
        ...
        self.use_skp_compatibility = compatibility.have_shape_keys_plus(ctx)
        self.dry_run = SyncProps.get_dry_run(ctx.scene)
...
def _execute(ctx: _SyncContext) -> list[str]:
    ...
    if SyncProps.get_lock_to_scene(ctx.source_scene):
...
def _run_with_skp(
    ...
    only_to_selected = SyncProps.get_skp_only_to_selected(ctx.source_key)
...
def _run_std(
    ...
    i_lock = SyncProps.get_ignore_locked(ctx.source_key)
...
def _build_shape_key_value_map_skp(ctx: _SyncContext) -> dict[str, float]:
    ...
    * `ignore_muted`: When enabled, shape keys on the source object that are
    ...
    i_mute = SyncProps.get_ignore_muted(key)
    stream: Iterable[ShapeKey]

    if SyncProps.get_skp_only_from_selected(key):
